...
class Movie:  # Class movie for movie related functions
    ...
    def movie_add(self):
        ...
        while True:
            try:
                ...
                print("Invalid input")
        if self.name not in finput.read():
            finput.close()
            fo = open("movies.txt", "a")
            fo.write("\n"+self.name)
            fo.close()
        else:
    ...
    def movie_search(self):
        finp = open("movies.txt", "r")
        if self.name in finp.read():
    ...
    def book_movie(self):
        ...
        while True:
            try:
                ...
                print("Invalid input")
        while True:
            if(re.match(r'[2-9]', str(total_ticket))):
                break
            else:
